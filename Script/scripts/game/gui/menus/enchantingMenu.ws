	...
	private function getEnchantmentsListData(nameList : array<CName>, type:int, out enchantmentsList:CScriptedFlashArray):void
	{
		...
		for (i = 0; i < itemsCount; i=i+1)
		{
			...
			if (schematic.schemName == 'Runeword 1')
				schematicLocDescription = "When a sign is cast, fills the sword with that sign's power for one strike. Costs 0.25 - 0.4 adrenaline points depending on the sign type.";
			else if (schematic.schemName == 'Glyphword 7')
				schematicLocDescription = "Igni base cast is circular instead of cone.";
			else {
